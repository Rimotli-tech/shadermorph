...
class MorphFrostedGlassTransition implements ShaderTransition {
  MorphFrostedGlassTransition._(this._program);

  static const String shaderAssetKey =
      'packages/shadermorph_flutter/assets/shaders/core/morph_frosted_glass.frag';

  final ui.FragmentProgram _program;

  static Future<MorphFrostedGlassTransition> load() async {
    final program = await ui.FragmentProgram.fromAsset(shaderAssetKey);
    return MorphFrostedGlassTransition._(program);
  }
  ...
  void configureShader(
    ...
    // Uniform Contract V1 float order. u_resolution must be the actual canvas size so
    // the shader can convert UV-space offsets into absolute pixel-space sampling.
